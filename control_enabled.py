...
cv2.createTrackbar("smoothness", "image" , 8, 20, nothing)   
...
while True:
    ...
    if enable_control:
        if click and not past_click:
            pyautogui.mouseDown()

        if past_click and not click:
            pyautogui.mouseUp()

        if absolute:
            pyautogui.moveTo(int(screenWidth * cursor_x), int(screenHeight * cursor_y), duration=0.0, tween=pyautogui.easeInOutQuad, _pause=False)
        else:
            pyautogui.move(sens * (cursor_x - 0.5), sens * (cursor_y - 0.5))
